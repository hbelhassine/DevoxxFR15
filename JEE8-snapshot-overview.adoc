...
** Des classes utilitaires permettent de mieux utiliser la nouvelle API Stream de Java 8 dans le traitement des données JSON
...
8. JMS 2.1 https://www.jcp.org/en/jsr/detail?id=368[(JSR 368)] : Proposer une alternative au **M**essage-**D**riven **B**ean avec une intégration CDI. Quelques corrections et évolutions mineures ont été apportées aussi.
...
Pour conclure, je constate que la spécification EJB n'a pas évolué et que CDI et JAX-RS représentent une alternative sérieuse et viable. Est-ce la fin des EJBs au profit des nouveaux nés ? Cette question a été posée au speaker et il n'a pas voulu confirmer cela. 
